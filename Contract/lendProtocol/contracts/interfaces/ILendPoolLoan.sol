...
interface ILendPoolLoan {
  ...
  event LoanRepaidInterceptorUpdated(
    address nftAsset,
    uint256 tokenId,
   ...
   */
  function approveLoanRepaidInterceptor(
    address interceptor,
    bool approved
  ) external;

  function isLoanRepaidInterceptorApproved(
    address interceptor
  ...
  function deleteLoanRepaidInterceptor(
    address nftAsset,
    uint256 tokenId
  ) external;

  function getLoanRepaidInterceptors(
    address nftAsset,
    uint256 tokenId
  ...
  function isFlashLoanLockerApproved(
    address locker
  ) external view returns (bool);
  ...
  function getLoanReserveBorrowScaledAmount(
    uint256 loanId
  ) external view returns (address, uint256);

  function getLoanReserveBorrowAmount(
    uint256 loanId
  ) external view returns (address, uint256);
  ...
  function getNftCollateralAmount(
    address nftAsset
  ) external view returns (uint256);

  function getUserNftCollateralAmount(
    address user,
    address nftAsset
  ) external view returns (uint256);
}
