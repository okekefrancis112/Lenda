...
library GenericLogic {
  ...
  function calculateLoanLiquidatePrice(
    ...
    (, vars.borrowAmount) = ILendPoolLoan(poolLoan).getLoanReserveBorrowAmount(
      loanId
    );
    ...
    vars.reservePriceInETH = IReserveOracleGetter(reserveOracle).getAssetPrice(
      reserveAsset
    );
  ...
  function calculateLoanBidFine(
    ...
    (, vars.debtAmount) = ILendPoolLoan(poolLoan).getLoanReserveBorrowAmount(
      loanData.loanId
    );
    ...
    returns (uint256 auctionEndTimestamp, uint256 redeemEndTimestamp)
  {
    ...
    if (
      (pauseDurationTime > 0) && (loanData.bidStartTimestamp <= pauseStartTime)
    ) {
